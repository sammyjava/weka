...
public class Util {
     ...
     */
    public static void printResults(double correct, double accuracy, double TPR, double FPR, double MCC, double precision, double d1, double d2) {
	System.out.println(round.format(correct)+"\t"+perc.format(accuracy)+"\t"+rate.format(TPR)+"\t"+rate.format(FPR)+"\t"+rate.format(MCC)+"\t"+rate.format(precision)+"\t"+
			   sci.format(d1)+"\t"+sci.format(d2));
    }
	...
	double MCC = evaluation.matthewsCorrelationCoefficient(1);
	double precision = evaluation.precision(1);
	printResults(correct, accuracy, TPR, FPR, MCC, precision, d1, d2);
    }
	...
	printResults(StatUtils.mean(corrects), StatUtils.mean(accuracys), StatUtils.mean(TPRs), StatUtils.mean(FPRs), StatUtils.mean(MCCs), StatUtils.mean(precisions));
	// best result
